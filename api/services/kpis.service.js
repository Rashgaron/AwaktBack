const { MotorBikes, Sites } = require('../models');

const getPurchases = async (kpiType, filter, year, zone, site) => {
    let data = await getMotorBikesData(); 
    switch (kpiType) {
        case 'most_bought':
...
const getMotorBikesData = async (zone, site) => {
    let result = [];
    if(zone){

    }else if(site){
        let siteData = await Sites.findById(site);
        siteData.motorBikes.map(async (x) => {
            result.push(await MotorBikes.findById(x));
        })
    }else 
        return await MotorBikes.find();
}
